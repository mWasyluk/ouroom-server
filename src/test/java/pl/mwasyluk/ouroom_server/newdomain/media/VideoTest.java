...
class VideoTest {
    ...
    Video newOf(MediaType mediaType, byte[] content) {
        return new Video(mediaType, content);
    }
    ...
    class ConstructorTest {
        ...
        void throwsExceptionWhenNullContentGiven() {
            assertThrowsExactly(NullPointerException.class, () -> newOf(VideoMediaType.MP4, null));
        }

        @Test
        @DisplayName("throws exception when empty content given")
        void throwsExceptionWhenEmptyContentGiven() {
            assertThrowsExactly(IllegalArgumentException.class, () -> newOf(VideoMediaType.MP4, new byte[0]));
        }
